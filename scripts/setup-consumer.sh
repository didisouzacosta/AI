...
ROOT=""; UPDATE=false; CANDIDATE=""; STAGE=""; OLD=""; BACKUP=""; MUTATED=false; GIT_DIR=""; SUB_HEAD=""; SUB_BRANCH=""; SUB_CREATED=false; LEDGER=""; PROJECT_LEDGER=""; CREATED_DIRS=(); CREATED_DIR_COUNT=0
...
cd "$ROOT"
safe_rel() { case "$1" in .codex/agents/*|.agents/skills/*|.swiftlint.yml|.swift-format|scripts/lint-swift.sh|scripts/test-required-class-marks.sh|.github/workflows/swift-lint.yml) ;; *) return 1;; esac; [[ "$1" != */ && "$1" != *//* && "$1" != */./* && "$1" != */../* && "$1" != *$'\n'* && "$1" != *$'\r'* ]]; }
...
for f in .swiftlint.yml .swift-format scripts/lint-swift.sh scripts/test-required-class-marks.sh templates/swift-lint.yml; do [ -f "$CANDIDATE/repo/$f" ] && [ ! -L "$CANDIDATE/repo/$f" ] || die "dependência sem $f"; done
