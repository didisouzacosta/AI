...
ROOT=""; UPDATE=false; ADOPT=false; ADOPT_LIST=""; CANDIDATE=""; STAGE=""; OLD=""; BACKUP=""; MUTATED=false; GIT_DIR=""; SUB_HEAD=""; SUB_BRANCH=""; SUB_CREATED=false; LEDGER=""; PROJECT_LEDGER=""; CREATED_DIRS=(); CREATED_DIR_COUNT=0
...
remove_created_directories() { local index; for ((index=CREATED_DIR_COUNT-1; index>=0; index--)); do rmdir "$ROOT/${CREATED_DIRS[$index]}" 2>/dev/null || true; done; }
cleanup() { local s="$?" path; if [ "$s" -ne 0 ] && [ "$MUTATED" = true ] && [ -n "$BACKUP" ]; then if [ "$SUB_CREATED" = true ]; then if [ -d "$ROOT/$SHARED" ] && [ -n "$(git -C "$ROOT/$SHARED" status --porcelain)" ]; then echo "Erro: rollback preservou alterações concorrentes no submodule criado." >&2; else rm -rf "$ROOT/$SHARED" "$GIT_DIR/modules/.ai/shared"; fi; elif [ -d "$ROOT/$SHARED" ]; then if [ -n "$(git -C "$ROOT/$SHARED" status --porcelain)" ]; then echo "Erro: rollback preservou alterações concorrentes no submodule." >&2; elif [ -n "$SUB_BRANCH" ]; then git -C "$ROOT/$SHARED" checkout -q -f "$SUB_BRANCH" || true; else [ -z "$SUB_HEAD" ] || git -C "$ROOT/$SHARED" checkout -q -f --detach "$SUB_HEAD" || true; fi; fi; if [ -f "$BACKUP/index" ]; then cp -p "$BACKUP/index" "$GIT_DIR/index"; else rm -f "$GIT_DIR/index"; fi; [ ! -f "$BACKUP/config" ] || cp -p "$BACKUP/config" "$GIT_DIR/config"; [ ! -f "$BACKUP/gitmodules" ] || cp -p "$BACKUP/gitmodules" "$ROOT/.gitmodules"; [ -f "$BACKUP/no-gitmodules" ] && rm -f "$ROOT/.gitmodules"; restore_project_ledger; restore_ledger; remove_created_directories; fi; [ -z "$CANDIDATE" ] || rm -rf "$CANDIDATE"; [ -z "$STAGE" ] || rm -rf "$STAGE"; [ -z "$OLD" ] || rm -f "$OLD"; [ -z "$BACKUP" ] || rm -rf "$BACKUP"; [ -z "$ADOPT_LIST" ] || rm -f "$ADOPT_LIST"; case "${AI_SETUP_REEXEC:-}" in "${TMPDIR:-/tmp}"/ai-setup.*) rm -f "$AI_SETUP_REEXEC";; esac; exit "$s"; }
trap cleanup EXIT
for arg in "$@"; do case "$arg" in --update) UPDATE=true;; --adopt-lint-config) ADOPT=true;; *) die "opção inválida: $arg";; esac; done
ROOT="$(git rev-parse --show-toplevel 2>/dev/null)" || die "execute dentro de um projeto Git consumidor."
GIT_DIR="$(git rev-parse --absolute-git-dir)"
cd "$ROOT"
# Lint/format files distributed to consumers (the workflow is published from templates/swift-lint.yml).
LINT_FILES=(.swiftlint.yml .swiftformat scripts/lint-swift.sh scripts/fix-swift-spacing.pl scripts/install-swift-tools.sh scripts/add-type-marks.py .github/workflows/swift-lint.yml)
# Files managed by earlier versions; still accepted in old manifests so updates can remove them.
LEGACY_FILES=(.swift-format scripts/test-required-class-marks.sh scripts/test-required-type-marks.sh scripts/test-swift-spacing.sh)
is_lint_file() { local file; for file in "${LINT_FILES[@]}"; do [ "$file" = "$1" ] && return 0; done; return 1; }
is_legacy_file() { local file; for file in "${LEGACY_FILES[@]}"; do [ "$file" = "$1" ] && return 0; done; return 1; }
lint_source() { [ "$1" = .github/workflows/swift-lint.yml ] && echo templates/swift-lint.yml || echo "$1"; }
safe_rel() { case "$1" in .codex/agents/*|.agents/skills/*) ;; *) is_lint_file "$1" || is_legacy_file "$1" || return 1;; esac; [[ "$1" != */ && "$1" != *//* && "$1" != */./* && "$1" != */../* && "$1" != *$'\n'* && "$1" != *$'\r'* ]]; }
...
publish_file() {
  ...
  parent="$(dirname "$rel")"; current=""; IFS=/ read -r -a parent_parts <<< "$parent"
  for component in "${parent_parts[@]}"; do current="${current:+$current/}$component"; if [ ! -e "$ROOT/$current" ]; then mkdir "$ROOT/$current"; CREATED_DIRS+=("$current"); CREATED_DIR_COUNT=$((CREATED_DIR_COUNT + 1)); fi; done
  parent="$ROOT/$parent"
...
adoptable() { local rel="$1"; [ "$ADOPT" = true ] && is_lint_file "$rel" || return 1; [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || return 1; [ ! -e "$ROOT/$rel.local-backup" ] && [ ! -L "$ROOT/$rel.local-backup" ] || die "$rel.local-backup já existe."; [ -n "$ADOPT_LIST" ] || ADOPT_LIST="$(mktemp "${TMPDIR:-/tmp}/ai-adopt.XXXXXX")"; printf '%s\n' "$rel" >> "$ADOPT_LIST"; }
is_adopted() { [ -n "$ADOPT_LIST" ] && grep -Fqx "$1" "$ADOPT_LIST"; }
adopt_local_file() { local rel="$1" hash; [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || die "arquivo adotado mudou: $rel"; cp -p "$ROOT/$rel" "$ROOT/$rel.local-backup"; record_project_file "$(shasum -a 256 "$ROOT/$rel.local-backup" | awk '{print $1}')" "$rel.local-backup"; hash="$(shasum -a 256 "$ROOT/$rel" | awk '{print $1}')"; echo "Configuração local adotada: $rel (backup em $rel.local-backup)" >&2; printf '%s\n' "$hash"; }
validate_manifest() {
...
for p in "$ROOT/.ai" "$ROOT/.codex" "$ROOT/.codex/agents" "$ROOT/.agents" "$ROOT/.agents/skills" "$ROOT/.claude" "$ROOT/.claude/skills"; do [ ! -L "$p" ] || die "diretório gerenciado não pode ser symlink."; [ ! -e "$p" ] || [ -d "$p" ] || die "diretório gerenciado inválido."; done
...
# ai-bootstrap/ai-update run the scripts of the local AI checkout, which may be older than main.
# Nothing has been mutated yet: rerun with main's setup script so the layout it expects is used.
if [ -z "${AI_SETUP_REEXEC:-}" ] && [ -f "$CANDIDATE/repo/scripts/setup-consumer.sh" ] && ! cmp -s "$CANDIDATE/repo/scripts/setup-consumer.sh" "${BASH_SOURCE[0]}"; then
  REEXEC_SCRIPT="$(mktemp "${TMPDIR:-/tmp}/ai-setup.XXXXXX")"; cp "$CANDIDATE/repo/scripts/setup-consumer.sh" "$REEXEC_SCRIPT"
  echo "Aviso: a base AI local está desatualizada; executando setup-consumer.sh de main. Atualize a base com git pull." >&2
  rm -rf "$CANDIDATE"; CANDIDATE=""; [ -z "$OLD" ] || rm -f "$OLD"; trap - EXIT
  AI_SETUP_REEXEC="$REEXEC_SCRIPT" exec bash "$REEXEC_SCRIPT" "$@"
fi
...
for rel in "${LINT_FILES[@]}"; do f="$(lint_source "$rel")"; [ -f "$CANDIDATE/repo/$f" ] && [ ! -L "$CANDIDATE/repo/$f" ] || die "dependência sem $f"; done
...
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/ai-stage.XXXXXX")"; (cd "$CANDIDATE/repo"; { LC_ALL=C find .codex/agents .agents/skills -type f -print; printf '%s\n' "${LINT_FILES[@]}"; } | LC_ALL=C sort) > "$STAGE/paths"
while IFS= read -r rel; do source_rel="$(lint_source "$rel")"; safe_rel "$rel" || die "fonte insegura."; safe_destination_parents "$rel" || die "destino inseguro: $rel"; mkdir -p "$STAGE/files/$(dirname "$rel")"; cp -p "$CANDIDATE/repo/$source_rel" "$STAGE/files/$rel"; printf '%s  %s\n' "$(shasum -a 256 "$CANDIDATE/repo/$source_rel" | awk '{print $1}')" "$rel" >> "$STAGE/manifest"; if [ -e "$ROOT/$rel" ] || [ -L "$ROOT/$rel" ]; then [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || die "destino inseguro."; managed_before "$rel" || cmp -s "$ROOT/$rel" "$STAGE/files/$rel" || adoptable "$rel" || die "conflito com arquivo local: $rel (use ai-update --adopt-lint-config para substituir configurações de lint locais com backup)"; fi; done < "$STAGE/paths"
LEGACY_FORMATTER=false; if [ "$ADOPT" = true ] && [ -f "$ROOT/.swift-format" ] && [ ! -L "$ROOT/.swift-format" ] && ! managed_before .swift-format; then [ ! -e "$ROOT/.swift-format.local-backup" ] && [ ! -L "$ROOT/.swift-format.local-backup" ] || die ".swift-format.local-backup já existe."; LEGACY_FORMATTER=true; fi
...
BACKUP="$(mktemp -d "${TMPDIR:-/tmp}/ai-rollback.XXXXXX")"; [ ! -d "$ROOT/$SHARED" ] || { SUB_HEAD="$(git -C "$ROOT/$SHARED" rev-parse HEAD)"; SUB_BRANCH="$(git -C "$ROOT/$SHARED" branch --show-current)"; }; for path in .ai .codex .agents .claude CLAUDE.md "${LINT_FILES[@]}" "${LEGACY_FILES[@]}" .swift-format.local-backup AGENTS.md AGENTS_backup.md; do [ ! -e "$ROOT/$path" ] && [ ! -L "$ROOT/$path" ] || { mkdir -p "$BACKUP/$(dirname "$path")"; cp -pR "$ROOT/$path" "$BACKUP/$path"; }; done; [ ! -f "$GIT_DIR/index" ] || cp -p "$GIT_DIR/index" "$BACKUP/index"; cp -p "$GIT_DIR/config" "$BACKUP/config"; if [ -f "$ROOT/.gitmodules" ]; then cp -p "$ROOT/.gitmodules" "$BACKUP/gitmodules"; else : > "$BACKUP/no-gitmodules"; fi; LEDGER="$BACKUP/ledger"; PROJECT_LEDGER="$BACKUP/project-ledger"; : > "$LEDGER"; : > "$PROJECT_LEDGER"; MUTATED=true
...
first_copy=true
while IFS= read -r rel; do expected="$(managed_hash "$rel" || true)"; if [ -n "$expected" ]; then verify_current_managed "$rel"; elif is_adopted "$rel"; then expected="$(adopt_local_file "$rel")"; elif [ -e "$ROOT/$rel" ] || [ -L "$ROOT/$rel" ]; then [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] && cmp -s "$ROOT/$rel" "$STAGE/files/$rel" || die "conflito com arquivo local: $rel"; fi; publish_file "$rel" "$STAGE/files/$rel" "$(shasum -a 256 "$STAGE/files/$rel" | awk '{print $1}')" "$expected"; if [ "$first_copy" = true ]; then first_copy=false; if [ -n "${AI_TEST_PAUSE_AFTER_FIRST_COPY:-}" ]; then : > "$AI_TEST_PAUSE_AFTER_FIRST_COPY/ready"; while [ ! -f "$AI_TEST_PAUSE_AFTER_FIRST_COPY/continue" ]; do sleep 0.05; done; fi; failpoint after-first-copy; fi; [ "$rel" != scripts/lint-swift.sh ] || failpoint after-lint-gate; failpoint during-copy; done < "$STAGE/paths"
...
if [ "$LEGACY_FORMATTER" = true ]; then mv "$ROOT/.swift-format" "$ROOT/.swift-format.local-backup"; printf 'V\t%s\t.swift-format\n' "$(shasum -a 256 "$ROOT/.swift-format.local-backup" | awk '{print $1}')" >> "$PROJECT_LEDGER"; echo "Formatter legado adotado: .swift-format movido para .swift-format.local-backup" >&2; fi
...
# Claude Code reads the same Agent Skills through .claude/skills/<name> symlinks and the
# rules through CLAUDE.md importing AGENTS.md. Existing consumer entries are preserved.
if [ ! -e "$ROOT/CLAUDE.md" ] && [ ! -L "$ROOT/CLAUDE.md" ]; then printf '# CLAUDE.md\n\n@AGENTS.md\n\nAGENTS.md é a fonte única das regras. As skills compartilhadas ficam em\n`.agents/skills/` e são expostas ao Claude Code por `.claude/skills/`.\n' > "$ROOT/CLAUDE.md"; record_project_file "$(shasum -a 256 "$ROOT/CLAUDE.md" | awk '{print $1}')" CLAUDE.md; fi
while IFS= read -r skill; do name="$(basename "$skill")"; link="$ROOT/.claude/skills/$name"; target="../../.agents/skills/$name"; if [ -L "$link" ] && [ "$(readlink "$link")" = "$target" ]; then continue; fi; if [ -e "$link" ] || [ -L "$link" ]; then echo "Aviso: .claude/skills/$name já existe e foi preservado." >&2; continue; fi; for dir in .claude .claude/skills; do [ -d "$ROOT/$dir" ] || { mkdir "$ROOT/$dir"; CREATED_DIRS+=("$dir"); CREATED_DIR_COUNT=$((CREATED_DIR_COUNT + 1)); }; done; ln -s "$target" "$link"; record_project_link "$target" ".claude/skills/$name"; done < <(find "$ROOT/.agents/skills" -mindepth 1 -maxdepth 1 -type d | LC_ALL=C sort)
if [ -d "$ROOT/.claude/skills" ]; then for link in "$ROOT/.claude/skills"/*; do [ -L "$link" ] || continue; case "$(readlink "$link")" in ../../.agents/skills/*) [ -e "$link" ] || { rm -f "$link"; echo "Link de skill removido: .claude/skills/$(basename "$link")" >&2; };; esac; done; fi
failpoint after-claude-skills
