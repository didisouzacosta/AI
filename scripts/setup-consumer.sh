#!/usr/bin/env bash
set -euo pipefail
REMOTE="${AI_BASE_REMOTE:-https://github.com/didisouzacosta/AI.git}"; SHARED=".ia/shared"
...
if [ -L "$ROOT/AGENTS.md" ]; then [ "$(readlink "$ROOT/AGENTS.md")" = ".ia/shared/AGENTS.md" ] || die "AGENTS.md é um symlink inesperado."; elif [ -e "$ROOT/AGENTS.md" ]; then [ -f "$ROOT/AGENTS.md" ] || die "AGENTS.md tem tipo especial."; [ ! -e "$ROOT/AGENTS_backup.md" ] && [ ! -L "$ROOT/AGENTS_backup.md" ] || die "AGENTS_backup.md já existe."; fi
for p in "$ROOT/.ia" "$ROOT/.codex" "$ROOT/.codex/agents" "$ROOT/.agents" "$ROOT/.agents/skills"; do [ ! -L "$p" ] || die "diretório gerenciado não pode ser symlink."; [ ! -e "$p" ] || [ -d "$p" ] || die "diretório gerenciado inválido."; done
...
OLD=""; if [ -e "$ROOT/.ia/managed-files.sha256" ] || [ -L "$ROOT/.ia/managed-files.sha256" ]; then OLD="$(mktemp "${TMPDIR:-/tmp}/ai-old.XXXXXX")"; validate_manifest "$ROOT/.ia/managed-files.sha256" "$OLD"; while IFS=$'\t' read -r hash rel; do safe_destination_parents "$rel" || die "caminho gerenciado inseguro: $rel"; [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || die "arquivo gerenciado inseguro: $rel"; [ "$(shasum -a 256 "$ROOT/$rel" | awk '{print $1}')" = "$hash" ] || die "arquivo gerenciado foi editado: $rel"; done < "$OLD"; fi
...
for f in AGENTS.md .ia/CODEX_ORCHESTRATOR.md .ia/SWIFT_REFERENCE.md .ia/PROJECT_BRIEF.template.md .ia/PROJECT_GUIDE.template.md; do [ -f "$CANDIDATE/repo/$f" ] && [ ! -L "$CANDIDATE/repo/$f" ] || die "dependência sem $f"; done
...
if [ ! -d "$ROOT/$SHARED" ]; then mkdir -p "$ROOT/.ia"; git submodule add -b main "$REMOTE" "$SHARED"; SUB_CREATED=true; elif [ "$UPDATE" = true ]; then git submodule update --remote -- "$SHARED"; else git submodule update --init -- "$SHARED"; fi
...
[ -e "$ROOT/AGENTS.md" ] || [ -L "$ROOT/AGENTS.md" ] || { ln -s ".ia/shared/AGENTS.md" "$ROOT/AGENTS.md"; record_project_link ".ia/shared/AGENTS.md" AGENTS.md; }
for f in PROJECT_BRIEF PROJECT_GUIDE; do if [ ! -e "$ROOT/.ia/$f.md" ]; then cp "$ROOT/$SHARED/.ia/$f.template.md" "$ROOT/.ia/$f.md"; record_project_file "$(shasum -a 256 "$ROOT/.ia/$f.md" | awk '{print $1}')" ".ia/$f.md"; fi; done
for f in CODEX_ORCHESTRATOR.md SWIFT_REFERENCE.md; do if [ ! -e "$ROOT/.ia/$f" ] && [ ! -L "$ROOT/.ia/$f" ]; then ln -s "shared/.ia/$f" "$ROOT/.ia/$f"; record_project_link "shared/.ia/$f" ".ia/$f"; fi; done
...
tmp_manifest="$(mktemp "$ROOT/.ia/.ai-managed-manifest.XXXXXX")"; cp -p "$STAGE/manifest" "$tmp_manifest"; mv -f "$tmp_manifest" "$ROOT/.ia/managed-files.sha256"; record_project_file "$(shasum -a 256 "$ROOT/.ia/managed-files.sha256" | awk '{print $1}')" .ia/managed-files.sha256
