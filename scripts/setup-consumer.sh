...
cd "$ROOT"
safe_rel() { case "$1" in .codex/agents/*|.agents/skills/*|.swiftlint.yml|.swiftformat|.swift-format|scripts/lint-swift.sh|scripts/fix-swift-spacing.pl|scripts/test-required-type-marks.sh|scripts/test-swift-spacing.sh|scripts/test-required-class-marks.sh|.github/workflows/swift-lint.yml) ;; *) return 1;; esac; [[ "$1" != */ && "$1" != *//* && "$1" != */./* && "$1" != */../* && "$1" != *$'\n'* && "$1" != *$'\r'* ]]; }
...
lint_config_rel() { case "$1" in .swiftlint.yml|.swiftformat|scripts/lint-swift.sh|scripts/fix-swift-spacing.pl|scripts/test-required-type-marks.sh|scripts/test-swift-spacing.sh|.github/workflows/swift-lint.yml) return 0;; *) return 1;; esac; }
adoptable() { local rel="$1"; [ "$ADOPT" = true ] && lint_config_rel "$rel" || return 1; [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || return 1; [ ! -e "$ROOT/$rel.local-backup" ] && [ ! -L "$ROOT/$rel.local-backup" ] || die "$rel.local-backup já existe."; [ -n "$ADOPT_LIST" ] || ADOPT_LIST="$(mktemp "${TMPDIR:-/tmp}/ai-adopt.XXXXXX")"; printf '%s\n' "$rel" >> "$ADOPT_LIST"; }
...
for f in .swiftlint.yml .swiftformat scripts/lint-swift.sh scripts/fix-swift-spacing.pl scripts/test-required-type-marks.sh scripts/test-swift-spacing.sh templates/swift-lint.yml; do [ -f "$CANDIDATE/repo/$f" ] && [ ! -L "$CANDIDATE/repo/$f" ] || die "dependência sem $f"; done
...
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/ai-stage.XXXXXX")"; (cd "$CANDIDATE/repo"; { LC_ALL=C find .codex/agents .agents/skills -type f -print; printf '%s\n' .swiftlint.yml .swiftformat scripts/lint-swift.sh scripts/fix-swift-spacing.pl scripts/test-required-type-marks.sh scripts/test-swift-spacing.sh .github/workflows/swift-lint.yml; } | LC_ALL=C sort) > "$STAGE/paths"
while IFS= read -r rel; do source_rel="$rel"; [ "$rel" != .github/workflows/swift-lint.yml ] || source_rel=templates/swift-lint.yml; safe_rel "$rel" || die "fonte insegura."; safe_destination_parents "$rel" || die "destino inseguro: $rel"; mkdir -p "$STAGE/files/$(dirname "$rel")"; cp -p "$CANDIDATE/repo/$source_rel" "$STAGE/files/$rel"; printf '%s  %s\n' "$(shasum -a 256 "$CANDIDATE/repo/$source_rel" | awk '{print $1}')" "$rel" >> "$STAGE/manifest"; if [ -e "$ROOT/$rel" ] || [ -L "$ROOT/$rel" ]; then [ -f "$ROOT/$rel" ] && [ ! -L "$ROOT/$rel" ] || die "destino inseguro."; managed_before "$rel" || cmp -s "$ROOT/$rel" "$STAGE/files/$rel" || adoptable "$rel" || die "conflito com arquivo local: $rel (use ai-update --adopt-lint-config para substituir configurações de lint locais com backup)"; fi; done < "$STAGE/paths"
