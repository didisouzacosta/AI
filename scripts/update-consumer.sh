...
[[ -d "$project/.ia/shared" ]] || { echo "Erro: .ia/shared não está configurado; execute ai-bootstrap." >&2; exit 1; }
cd "$project"
bash "$root/scripts/setup-consumer.sh" --update
echo "Dependência AI atualizada. Revise e registre .ia/shared, .ia/managed-files.sha256, .codex e .agents."
